...
// const ENEMY_SPAWN_RATE_MS = 2500; // REMOVED: No longer timed spawns
...
const BOOMERANG_ENEMY_HEALTH = 12;
const BOOMERANG_ENEMY_DAMAGE = 2;
const BOOMERANG_ENEMY_CHASE_SPEED = 120;

// --- NEW BOSS CONSTANTS ---
const BOSS_LEVEL_INTERVAL = 30; // Boss spawns every 30 waves
const BOSS_BASE_HEALTH = 150;
const BOSS_DAMAGE = 5;
const BOSS_SHOOT_RATE_MS = 1500;
const BOSS_BULLET_SPEED = 500;
// const BOSS_XP_REWARD = 100; // REMOVED: No XP

// const MAX_ENEMIES = 150; // REMOVED: Max enemies is now per-wave
const MAX_BOOMERANG_ENEMIES = 3; // Kept as a per-wave cap

// --- MODIFIED: Smaller map size ---
...
class Enemy extends Phaser.Physics.Arcade.Image {
    ...
    takeDamage(amount, isCrit = false) {
        if (!this.active) return [false, 0]; // MODIFIED: Return [isDead, damageTaken]

        let damageTaken = 0; // NEW: Track actual damage dealt
        ...
        if ((this.getData('isElite') || this.getData('isBoss')) && newArmor > 0) {
            ...
            damageTaken += damageToArmor; // Add damage dealt to armor
        }

        // 2. Damage Health
        if (remainingDamage > 0) {
            const damageToHealth = Math.min(remainingDamage, newHealth); // Don't overkill health
            newHealth = newHealth - damageToHealth;
            this.setData('health', newHealth);
            damageTaken += damageToHealth; // Add damage dealt to health
        }

        if (newHealth <= 0) {
            this.kill();
            return [true, damageTaken]; // MODIFIED: Return isDead=true and damage dealt
        }
        ...
        return [false, damageTaken]; // MODIFIED: Return isDead=false and damage dealt
    }
    ...
    kill() {
        ...
        // --- NEW: Notify the scene that an enemy was killed ---
...
class MainScene extends Phaser.Scene {
    constructor() {
        ...
        this.bullets = null;
        this.enemies = null;
        this.score = 0; // Score is now 1 per kill
        ...
        // --- NEW: Wave-based stats ---
        this.waveNumber = 0; // Starts at 0, first wave is 1
        this.enemiesLeftInWave = 0;
        this.waveTotalEnemies = 0;
        // --- END NEW ---

        // --- NEW: Timer and Damage Tracking ---
    ...
    create() {
        this.score = 0;
        this.waveNumber = 0; // Will be set to 1 by startNextWave
        this.playerHealth = PLAYER_HEALTH;
        this.playerMaxHealth = PLAYER_HEALTH;
        this.playerSpeed = PLAYER_BASE_SPEED;
        ...
        // --- NEW: Reset wave stats ---
        this.enemiesLeftInWave = 0;
        this.waveTotalEnemies = 0;

        // --- NEW: Reset timer and damage ---
        ...
        // REMOVED: expOrbs group and overlap
        ...
        // REMOVED: Timed enemy spawner

        // --- NEW: Start the first wave ---
        this.startNextWave();
        // --- END NEW ---
        ...
        console.log("Phaser Game Created. New WAVE-BASED System Initialized.");
    }
    ...
    // --- UPDATED: sendFullStats (includes Boss UI data, new Wave data, and elapsed time) ---
    sendFullStats() {
        ...
        // --- NEW: Send all stats + Boss data + Wave progress ---
        this.onUpdate({
            type: 'fullStats',
            waveNumber: this.waveNumber, // CHANGED from level
            ...
            score: this.score,
            elapsedTime: elapsedTime, // NEW
            // --- REMOVED: Wave Stats (UI doesn't show them) ---
            // --- BOSS STATS ---
    ...
    update(time) {
        ...
        if (!this.physics.world.isPaused) {
            ...
            // Re-send stats on every frame to ensure boss direction marker and timer are correct
            this.sendFullStats();
        } else {
             if (this.thrusterEmitter.emitting) this.thrusterEmitter.stop();
        }
    ...
    // --- UPDATED: applyUpgrade now starts the next wave ---
    applyUpgrade(choice) {
        ...
        // --- NEW: Start the next wave after picking an upgrade ---
        this.startNextWave();
    }
    ...
    bossFire() {
        ...
        if (bullet) {
            // The angle is calculated only once, when the bullet is fired (non-tracking)
            const angle = Phaser.Math.Angle.Between(
                this.boss.x, this.boss.y,
                this.player.x, this.player.y
            );
    ...
    spawnRegularEnemy(x, y) {
        ...
        if (enemy) {
            const levelBonus = this.waveNumber - 1; // Use waveNumber
    ...
    spawnBoss(x, y) {
        ...
        if (boss) {
            ...
            const levelMultiplier = Math.floor(this.waveNumber / BOSS_LEVEL_INTERVAL); // Use waveNumber
            const levelBonus = this.waveNumber - 1; // Use waveNumber
            ...
            // --- NEW: Set wave enemy count to 1 for the boss ---
    ...
    // --- UPDATED: Spawn Wave (now spawns the whole wave based on waveNumber) ---
    spawnWave() {
        if (this.isGameOver || this.isBossActive) return; // Don't spawn if boss is active
        ...
        // --- NEW: Calculate wave composition ---
        const baseWaveSize = 5;
        this.waveTotalEnemies = baseWaveSize + (this.waveNumber - 1) * 2; // e.g., 5, 7, 9...
        this.enemiesLeftInWave = this.waveTotalEnemies;

        let eliteChance = 0.05 + (this.waveNumber * 0.005); // Increases slowly
        eliteChance = Math.min(0.35, eliteChance); // Cap at 35%

        let boomerangChance = 0.1 + (this.waveNumber * 0.002); // Increases very slowly
        boomerangChance = Math.min(0.25, boomerangChance); // Cap at 25%

        this.boomerangEnemiesCount = 0; // Reset per-wave cap
    ...
    // --- MODIFIED: handleGameOver to send new stats ---
    handleGameOver() {
        ...
        // --- NEW: Calculate final time and send all stats ---
    ...
    // --- HEAVILY UPDATED: hitEnemy (No orb logic, track damage) ---
    hitEnemy(bullet, enemy) {
        ...
        // 2. Apply Damage and get actual damage dealt
        const [isDead, damageDealt] = enemy.takeDamage(finalDmg, isCrit); // MODIFIED

        // --- NEW: Track total damage ---
        this.totalDamageDealt += damageDealt;
        // --- END NEW ---

        // 3. Handle Orb Drop
        if (isDead) {
            // REMOVED: Orb drop logic
        }
    ...
    // --- UPDATED: hitEnemyByShield (No orb logic, track damage) ---
    ...
    // --- UPDATED: enterUpgradeState (Now just shows the menu) ---
    enterUpgradeState() {
        console.log(`[Phaser] Wave ${this.waveNumber} complete. Showing upgrades.`);

        // Stop the game and show the upgrade menu
        const choices = this.generateUpgradeChoices();
        this.onShowUpgrade(choices);
        this.setInvulnerable(this.player, -1); // Invulnerable during upgrade choice

        // The game will resume (and start next wave) when applyUpgrade is called
    }

    // --- UPDATED: Upgrade Choice Generation (uses waveNumber) ---
    ...
    startNextWave() {
        this.waveNumber++; // Increment wave
        console.log(`[Phaser] Starting Wave ${this.waveNumber}`);

        // --- NEW: Send update to React to show "WAVE X" ---
        ...
        if (isBossRound) {
            ...
            } while (Phaser.Math.Distance.Between(x, y, this.player.x, this.player.y) < bossSpawnDistance);

            this.spawnBoss(x, y); // spawnBoss now sets enemy counts

        } else if (this.isBossActive) {
            // This case handles finishing a boss
            // The boss is already cleared, just spawn a regular wave
            console.warn("Boss defeated, starting next regular wave.");
            this.isBossActive = false;
            if(this.boss) this.boss = null;
            this.spawnWave(); // Spawn regular wave
        } else {
            // Regular wave
            this.spawnWave();
        }
    ...
    // --- NEW: Function to handle enemy death ---
    onEnemyKilled(enemy) {
        if (this.isGameOver) return;

        // Only count non-boss kills for score
        if (!enemy.getData('isBoss')) {
            this.score += 1; // Give 1 score per kill
            this.onUpdate({ type: 'score', value: this.score });
        }

        this.enemiesLeftInWave = Math.max(0, this.enemiesLeftInWave - 1);

        // Send stats to update UI (e.g., "Enemies Left: 4/10")
        // this.sendFullStats(); // No longer needed as enemy count isn't displayed
    ...
    // --- UPDATED: Zap Enemies (No orb logic, track damage) ---
    zapEnemies() {
        ...
        for (let i = 0; i < electricChains; i++) {
            ...
            const [isDead, damageDealt] = currentTarget.takeDamage(finalDmg, isCrit); // MODIFIED
